...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch all photograph IDs associated with the photographer
    const photographs = await prisma.photograph.findMany({
      where: { photographer_id: photographerId },
      select: { id: true },
    });
    ...
    // If no photographs are found, return empty labels and values
    if (photographIds.length === 0) {
      res.setHeader('Cache-Control', 'no-store');
      return res.status(200).json({ labels: [], values: [] });
    }
    ...
    let queryResult;
    if (viewsType === 'total') {
      // Total Views: Count all PhotoViewEvent grouped by the date part of createdAt
      queryResult = await analyticsPrisma.$queryRaw(Prisma.sql`
        SELECT
          DATE(createdAt) AS theDate,
          COUNT(*) AS total
